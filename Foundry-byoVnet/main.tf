  ...
  body = {
    ...
    properties = {
      ...
      networkInjections = [
        {
          scenario                   = "agent"
          subnetArmId                = data.terraform_remote_state.networking.outputs.ai_foundry_subnet00_id
...
resource "azurerm_role_assignment" "cosmosdb_operator_ai_foundry_project" {
  ...
  name                 = uuidv5("dns", "${azapi_resource.ai_foundry_project.name}${azapi_resource.ai_foundry_project.output.identity.principalId}${azurerm_resource_group.rg-ai00.name}cosmosdboperator")
