...
- `outbound_only` apply completes, but Fabric can return a generic `UnknownError` while deleting outbound resources. Repeated destroy attempts can make partial progress and eventually complete.
...
2. In the Fabric workspace inbound networking settings, set public access to `Allow`.
...
This temporary access change prevents the Fabric provider refresh from failing before Terraform can delete the workspace. If cleanup is abandoned, restore the inbound policy to `Deny`.
...
If `UnknownError` continues after bounded retries or an extended wait, capture the full Terraform resource address, request ID, and local debug log for Microsoft support or a Fabric provider issue.
...
- Confirm the guidance preserves state and request IDs.
